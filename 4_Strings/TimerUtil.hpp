...
#ifndef TIMERUTIL_HPP
#define TIMERUTIL_HPP
...
class Timer {
  public:
    ...
    explicit Timer(std::string componentName) : componentName(std::move(componentName)){};
    ...
    void start() {
        if (running) {
            std::cout << "Timer: Trying to start an already running timer so will skip this operation\n";
        } else {
            running = true;
    ...
    void snapshot(std::string snapshotName) {
        if (!running) {
            std::cout << "Timer: Trying to take a snapshot of an non-running timer so will skip this operation\n";
        } else {
            running = true;
            stop_p = ClockType::now();
            auto duration = std::chrono::duration_cast<TimeUnit>(stop_p - start_p);
    ...
    void merge(Timer timer) {
        if (running) {
            std::cout << "Timer: Trying to merge while timer is running so will skip this operation\n";
        } else {
     ...
     */
    [[nodiscard]] int64_t getRuntime() const {
        if (!running) {
            return runtime.count();
        } else {
            std::cout << "Timer: Trying get runtime while timer is running so will return zero\n";
            return 0;
        }
    };
    ...
    */
    [[nodiscard]] const std::string& getComponentName() const { return componentName; };
